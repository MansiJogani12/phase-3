...
# 1. Replace the closing div of the tabs container with the new tabs + closing div
content = re.sub(r'<TabButton name="security"([^>]+)>\s*</div>', r'<TabButton name="security"\1>' + tabs_to_add, content)
...
# 2. Add Top Header
content = re.sub(r'<div className="bg-white border-2 border-black rounded-xl shadow-\[8px_8px_0px_rgba\(0,0,0,1\)\]\s+h-full flex flex-col overflow-hidden">', top_header_ui, content)
